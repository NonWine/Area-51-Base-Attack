...
public class ShokerTile : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.transform.CompareTag("Dire") && other != this.other)
        {
            ...
            {
                Unit unit = tardetDetected.GetComponent<Unit>();
                if(unit.GetType() == Mob.Mimic)
                    rd.velocity = ((new Vector3(tardetDetected.position.x, 5f, tardetDetected.position.z) -  new Vector3(transform.position.x,5f, transform.position.z) )).normalized * _speed;
                else if(unit.GetType() == Mob.Runner)
                    rd.velocity = ((new Vector3(tardetDetected.position.x, 2f, tardetDetected.position.z) - new Vector3(transform.position.x, 2f, transform.position.z))).normalized * _speed;
                else
                ...
                other.transform.GetComponent<Unit>().GetDamage(dmg);
                currentRc++;
                inTrigger = false;
                tardetDetected = null;
    ...
    public void TryFindTheNearlestEnemy()
    {
        ...
        if (inTrigger)
            ...
            {
                if (enemys[i].CompareTag("Dire") && (( enemys[i].gameObject != other.gameObject ) && !enemys[i].GetComponent<Unit>().isDead()))
                {
